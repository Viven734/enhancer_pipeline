KM <- function(input_dir, output_dir) {
  ...
  library("survival")
  setwd(input_dir)
  data = read.table("riskScore_combined_eRNA.txt", header = T, sep = "\t", check.names = F, row.names = 1)
  # Fit survival curves
  fit <- survfit(Surv(OS, Censor) ~ risk3, data = data)
  ...
  ggsave(paste(output_dir, "/Combined_set.pdf", sep = ''), plot = p$plot)
